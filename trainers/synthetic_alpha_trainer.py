...
class Trainer: 
    ...
    def _train_session(self, session, x):
        ...
        # forward
        embedded = embedding(afferents)        # [batch x dim x time]
        # embedded = embedded.permute([2,0,1])    # [time x batch x dim]
        motor_neurons = self.core(embedded)  # [batch x dim x time]
    ...
    def _valid_session(self, iteration, session, is_save=False):  
        ...
        list_loss = []
        if is_save:
            list_emg, list_predicted_emg = [], []

        for afferents, emg in valid_loader:
            ...
            if self.is_sliding_window:
                batch_size, n_channels = afferents.size(0), afferents.size(1)
                afferents = F.unfold(afferents[:,:,:,None], kernel_size=(self.window_size,1), stride=(self.stride,1))    # Bx(CxW)xN
                afferents = afferents.view(batch_size, n_channels, self.window_size, -1)        # BxCxWxN
                n_windows = afferents.size(3)
                afferents = afferents.permute([0,3,1,2])                                    # BxNxCxW
                afferents = afferents.contiguous().view(-1, n_channels, self.window_size)       # (BxW)xCxW
